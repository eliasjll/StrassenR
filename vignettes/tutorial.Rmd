...
For my project, I decided to implement Strassen's algorithm for high-performance matrix multiplication in R, with the help of a generative AI partner, Gemini CLI. This tutorial documents the entire journey, from initial concept to a fully optimized and parallelized R package.
...
To make this fast, I needed to implement it in C++ using Rcpp. As a C++ novice, I relied on Gemini CLI. I first wrote the algorithm in pure R, then asked the AI to translate it.
...
After several iterations, I created a `Makevars` file that worked on both macOS and Linux, a critical step for making the package portable.
...
In conclusion, I successfully built and optimized an R package that outperforms R's built-in operator, using Gemini CLI as a development and debugging partner. I used Strassen's because it is not a "galactic" algorithm (while all the algorithms with a smaller timne complexity were), meaning its performance gains are visible on practical problem sizes.
